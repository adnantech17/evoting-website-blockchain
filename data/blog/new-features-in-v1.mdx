...
A demo of a multiple authors post is shown in [Introducing Tailwind Nextjs Starter Election post](/blog/introducing-tailwind-nextjs-starter-blog).
...
Two components are exported, a default `NewsletterForm` and a `ElectionNewsletterForm` component, which is also passed in as an MDX component
and can be used in a blog post:

```jsx
<ElectionNewsletterForm title="Like what you are reading?" />
```
