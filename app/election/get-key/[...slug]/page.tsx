...
import { getKey, performVote } from 'client/elections'
...
export default function Page({ params }: { params: { slug: string[] } }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await getKey(params.slug, { username: id, password: fingerprint })
      localStorage.setItem('key', response.data)
      router.push(`/election/key-success/${params.slug}`)
    } catch (error) {
      console.error(error)
